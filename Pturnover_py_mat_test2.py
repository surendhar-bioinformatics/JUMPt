import numpy as np, scipy.optimize as so, scipy.integrate as si, pandas as pd, de, numpy.linalg as nl, coupledOde as co, scipy.sparse as ss, generateUnobs as gu, matplotlib.pyplot as plt, time, subprocess
from datetime import datetime
print("This is Global optimization program to fit the proteins")

def runMatlab( scriptFile ):
    """
    Run matlab on scriptFile and return when finished.
    """
    retval = subprocess.run( ['matlab', '-nodisplay', '-nosplash',  '-minimize' ,'-r', scriptFile ] )
...
# ============================ function to Print Minimization at each step ===========================
...
Inputfile   = "min3_data_UIP.xlsx" # Assign spreadsheet filename to `file`
P_data      = pd.read_excel(Inputfile,sheet_name= 'min3data_orig') # Load spreadsheet
...
plt.savefig('Results/Original_data.png', bbox_inches='tight')
...
mask    = (1 - np.isnan(residual)).nonzero()
flatResidual = residual[mask]; ResError = nl.norm(flatResidual)**2;
print('LSE after derivative fitting = ', ResError)
...
print('\n Calling MATLAB program to print simulated trajectories for Globally optimized paramters ...')

#runMatlab( 'FinalPlot' )

print('\n Completed generating trajectories and figure results were saved')

# Below is Python Code for Plotting
"""
Inputfile   = Path("Results\Glob_fit_res.xlsx") # Assign spreadsheet filename to `file`
GlobFitRes  = pd.read_excel(Inputfile,sheet_name= 'Glob_fit') # Load spreadsheet
gamma_Lys_P_GlobFit   = (GlobFitRes.loc[:, 'gamma']).values; 
number_param = len_P_data + 2 
startTime2 = datetime.now()
EtaP_UIP = np.hstack((EtaP, (sum_EtaP - sum(EtaP))))
y0 = np.ones(number_param);


A1_P_ratio_simu = si.solve_ivp(lambda t, y: PT_ODE(t, y, gamma_Lys_P_GlobFit, A1, EtaP_UIP), [0, 32], y0, method="RK23", t_eval=tspan); 
LSE_A1 = ((np.transpose(A1_ratio) - A1_P_ratio_simu.y[0,:])**2).sum() ;
LSE_A1_Pro = LSE_A1 + np.nansum((np.transpose(P_ratio) - A1_P_ratio_simu.y[1:,:])**2);  print('LSE_Lys = ', LSE_A1, 'LSE_Lys_Pro = ', LSE_A1_Pro)
startTime2 = datetime.now()

A1_P_ratio_simu = si.solve_ivp(lambda t, y: PT_ODE(t, y, gamma_Lys_P_GlobFit, A1, EtaP_UIP), [0, 32], y0, method="RK45", t_eval=tspan_medium);
print('Time for calling solve ivp function is = ', datetime.now() - startTime2, '(hr:min:sec) \n')

plt.figure(1)
plt.plot(tspan, A1_ratio,     'rs',linewidth=2, markeredgecolor='r',markeredgewidth=2, markersize=6,label='A_L- (Lys unbound)')
plt.plot(justify(tspan_data[:,0], invalid_val=np.nan, axis=0, side='left'),justify(P_ratio[:,0],invalid_val=np.nan, axis=0, side='left'), 'bo',linewidth=2, markeredgecolor='b',markeredgewidth=1, markersize=6,label='P_1')
plt.plot(justify(tspan_data[:,1], invalid_val=np.nan, axis=0, side='left'),justify(P_ratio[:,1],invalid_val=np.nan, axis=0, side='left'), 'go',linewidth=2, markeredgecolor='g',markeredgewidth=1, markersize=6,label='P_2')
plt.plot(justify(tspan_data[:,2], invalid_val=np.nan, axis=0, side='left'),justify(P_ratio[:,2],invalid_val=np.nan, axis=0, side='left'), 'co',linewidth=2, markeredgecolor='c',markeredgewidth=1, markersize=6,label='P_3')
plt.plot(tspan_medium,A1_P_ratio_simu.y[0,:], 'r-',linewidth=2, markeredgecolor='r',markeredgewidth=1, markersize=6,label='A_L')
plt.plot(tspan_medium,A1_P_ratio_simu.y[1,:], 'b-',linewidth=2, markeredgecolor='b',markeredgewidth=1, markersize=6,label='P_1')
plt.plot(tspan_medium,A1_P_ratio_simu.y[2,:], 'g-',linewidth=2, markeredgecolor='g',markeredgewidth=1, markersize=6,label='P_2')
plt.plot(tspan_medium,A1_P_ratio_simu.y[3,:], 'c-',linewidth=2, markeredgecolor='c',markeredgewidth=1, markersize=6,label='P_3')
plt.title('Original data and simulations',FontSize = 16)
plt.xlabel('Time (days)', FontSize = 14); plt.ylabel('Fraction of  \'light\'  Lys ', FontSize=14)
plt.legend(loc='best'); plt.ylim(0, 1.03); plt.show()
print('Time for excution to read data is = ', datetime.now() - startTime, '(hr:min:sec)')
"""
