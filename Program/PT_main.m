...
if any(mask2)
    ...
    data.protInfo  = (All_data(2:end,1:find(mask,1)-1)); % protein IDs
    data.ProtConc  = table2array(All_data(2:end,mask3))'; %Protein concentration
    SILAC_data      = table2cell(All_data(2:end,mask));% protein data
    SILAC_data_NaN  = cellfun(@(x) ~isa(x,'double'),SILAC_data); % check for possible 0x0 char reads
    SILAC_data(SILAC_data_NaN)   = {NaN};% set those to NaN
    data.SILAC_data              = cell2mat(SILAC_data)'; % heavy/light ratios,
end
